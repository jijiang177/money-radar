...
async function callAIWithRetry(payload, apiKey) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      const resp = await axios.post('https://api.deepseek.com/v1/chat/completions', { ...payload, model: process.env.DEEPSEEK_MODEL || 'deepseek-chat' }, { headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, timeout: 60000 });
      return resp.data;
    } catch (err) {
      lastError = err;
      const status = err.response?.status;
      if (status && status < 500) break;
      if (attempt < 3) { const delay = 1000 * (2 ** (attempt - 1)); console.warn(`  [AI] 重试 ${attempt}/3，等待 ${delay}ms...`); await new Promise(r => setTimeout(r, delay)); }
    }
  }
  throw lastError;
}
...
async function processBatch(batch, apiKey) {
  const contentText = batch.map((item, i) => `[${i + 1}] 平台: ${item.platform}\n标题: ${item.title}\n内容: ${item.content}\n链接: ${item.url}`).join('\n---\n');
...
[
  { "painPoint": "用户具体的痛点描述", "toolIdea": "建议做的微型 H5 工具描述", "category": "工具类型", "sourceIndex": 编号, "score": 0-5, "reason": "评分理由 + 变现方式" }
]
如果确实没有任何有价值的需求（score < 3），返回 []`;

  const data = await callAIWithRetry({ messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: `请分析以下用户讨论内容，识别出可以被做成微型 H5 工具的需求：\n\n${contentText}` }], temperature: 0.5, max_tokens: 2000 }, apiKey);
  ...
  return batchInsights.map(item => {
    ...
    return { painPoint: item.painPoint, toolIdea: item.toolIdea, category: item.category || '其他', score: item.score ?? 0, reason: item.reason || '', sourceTitle: source.title || '', sourceContent: source.content || '', sourceUrl: source.url || '', sourcePlatform: source.platform || '' };
  });
}

async function analyzeWithDeepSeek(rawContents) {
  const apiKey = process.env.DEEPSEEK_API_KEY;
  if (!apiKey) { console.warn('[AI] ⚠ 未配置 DEEPSEEK_API_KEY，使用本地规则过滤'); return localFilter(rawContents); }
  ...
  if (preFiltered.length === 0) { console.log('[AI] 预过滤后无内容，跳过 API 调用'); return []; }
  console.log('[AI] 第二步：DeepSeek 深度分析（并行批量）...');
  const BATCH_SIZE = 10;
  const batches = [];
  for (let i = 0; i < preFiltered.length; i += BATCH_SIZE) { batches.push(preFiltered.slice(i, i + BATCH_SIZE)); }
  console.log(`[AI] 分 ${batches.length} 批并行调用 API...`);
  const batchResults = await Promise.allSettled(batches.map(async (batch, idx) => { console.log(`[AI] 启动第 ${idx + 1}/${batches.length} 批 (${batch.length} 条)...`); const insights = await processBatch(batch, apiKey); console.log(`  第 ${idx + 1} 批识别出 ${insights.length} 个需求`); return insights; }));
  const allInsights = [];
  for (const result of batchResults) { if (result.status === 'fulfilled') allInsights.push(...result.value); }
...
function localFilter(rawContents) {
  ...
  for (const item of rawContents) {
    ...
    if (painKeywords.some(k => text.includes(k)) || toolKeywords.some(k => text.includes(k))) {
      results.push({ painPoint: item.content.substring(0, 200) || item.title, toolIdea: generateToolIdea(item.title, item.content), reason: '本地规则匹配：包含需求关键词', sourceTitle: item.title, sourceContent: item.content, sourceUrl: item.url, sourcePlatform: item.platform });
    }
  }
...
function generateToolIdea(title, content) {
  ...
  if (text.includes('菜谱') || text.includes('做饭') || text.includes('冰箱')) return '做一个冰箱有啥做啥 H5，输入现有食材，AI 推荐可做的菜谱';
...
module.exports = { analyzeWithDeepSeek, localFilter };
